...
def run_column_space_test(matrix_M, vector_b, matrix_M_basis, test_name):#TD done
    print(f"\n[TEST CASE: {test_name} ]")
    ...
    if in_column_space(matrix_M, vector_b):
        ...
        if in_column_space(matrix_M_basis, vector_b):
            print(f"Verification: PASS (The vector is successfully spanned by the basis).")
        else:
...
def run_null_space_test(matrix_M, vector_b, matrix_M_basis, test_name):#TD done
    print(f"\n[TEST CASE: {test_name}]")
